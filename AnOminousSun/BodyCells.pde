class BodyCells implements AppletInterface
{
  ...
  class Cloud {
    ...
    boolean puller_wrapping = false; 
    boolean puller_bouncing = true; 
    boolean puller_locked = true;
    boolean pusher_wrapping = false; 
    boolean pusher_bouncing = true; 
    boolean pusher_locked = true;
    float speed = 0.1; 
    float wander_speed = 0.1; 
    float attract_distance = 80; 
    float attract_speed = 0.0006; 
    float repel_distance = 40; 
    float repel_speed = 0.05;
    float puller_speed = 0.005; 
    float puller_wander_speed = 0.01; 
    float puller_attract_distance = 200; 
    float puller_attract_speed = 0.0007; 
    float puller_repel_distance = 30; 
    float puller_repel_speed = 0.07;
    float pusher_speed = 0.005; 
    float pusher_wander_speed = 0.01; 
    float pusher_attract_distance = 0; 
    float pusher_attract_speed = 0.0005; 
    float pusher_repel_distance = 60; 
    float pusher_repel_speed = 0.08;
    float gx = 0; 
    float gy = 0; 
    float gz = 0;
    float friction = 0.1;
    ...
    void add_particle(Particle $p) {
      ...
      $p.bouncing = bouncing;
      $p.speed = speed; 
      $p.wander_speed = wander_speed; 
      $p.attract_distance = attract_distance; 
      $p.attract_speed = attract_speed; 
      $p.repel_distance = repel_distance; 
      $p.repel_speed = repel_speed;
      $p.gx = gx; 
      $p.gy = gy; 
      $p.gz = gz; 
      $p.friction = friction; 
      $p.bounce_friction = bounce_friction;
      particles[nparticles++] = $p;
    }
    void add_puller(Particle $p) {
      $p.wrapping = puller_wrapping; 
      $p.bouncing = puller_bouncing; 
      $p.locked = puller_locked;
      $p.speed = puller_speed; 
      $p.wander_speed = puller_wander_speed; 
      $p.attract_distance = puller_attract_distance; 
      $p.attract_speed = puller_attract_speed; 
      $p.repel_distance = puller_repel_distance; 
      $p.repel_speed = puller_repel_speed;
      $p.gx = gx; 
      $p.gy = gy; 
      $p.gz = gz; 
      $p.friction = friction; 
    ...
    void add_pusher(Particle $p) {
      $p.wrapping = pusher_wrapping; 
      $p.bouncing = pusher_bouncing; 
      $p.locked = pusher_locked;
      $p.speed = pusher_speed; 
      $p.wander_speed = pusher_wander_speed; 
      $p.attract_distance = pusher_attract_distance; 
      $p.attract_speed = pusher_attract_speed; 
      $p.repel_distance = pusher_repel_distance; 
      $p.repel_speed = pusher_repel_speed;
      $p.gx = gx; 
      $p.gy = gy; 
      $p.gz = gz; 
      $p.friction = friction; 
  ...
  class Delaunay {
    ...
    void add_face(Point $p) {
      for (int i=0; i<npoints; i++) {
        if ($p.get_distance_to(points[i])<=global.distance_tolerance) { 
          return;
        }
      }
      ...
      for (int i=0; i<nsegments; i++) {
        ...
        for (int j=0; j<nsegments; j++) {
          if (segments[i].is_identical(segments[j])) { 
            duplicates++;
          }
        }
        if (duplicates==1) {
  ...
  class Face extends Polygon {
    ...
    boolean is_acute() {
      ...
      d1 = p1.get_distance_to(p2); 
      d2 = p2.get_distance_to(p3); 
      d3 = p3.get_distance_to(p1);
      x = (sq(d1)+sq(d2)-sq(d3))/(2*d2); 
      h = sqrt(sq(d1)-sq(x)); 
      y = d2-x;
      a1 = (atan(h/x)+PI)%PI; 
      a2 = (atan(h/y)+PI)%PI; 
      a3 = PI-(a1+a2);
      if (a1<HALF_PI&&a2<HALF_PI&&a3<HALF_PI) { 
        return true;
      }
      return false;
    }
    ...
    Point get_incenter_point() {
      float d1, d2, d3; 
      Point p;
      d1 = p2.get_distance_to(p3); 
      d2 = p3.get_distance_to(p1); 
      d3 = p1.get_distance_to(p2);
    ...
    Point get_circumcenter_point() {
      Vector p12, p23, n12, n23; 
      Point mid12, mid23; 
      Line l12, l23; 
      Segment s;
      pl.nv = pl.get_normal();
      p12 = new Vector(p2.x-p1.x, p2.y-p1.y, p2.z-p1.z); 
      p23 = new Vector(p3.x-p2.x, p3.y-p2.y, p3.z-p2.z);
      n12 = cross_product(pl.nv, p12); 
      n23 = cross_product(pl.nv, p23);
      mid12 = new Point((p2.x+p1.x)/2, (p2.y+p1.y)/2, (p2.z+p1.z)/2); 
      mid23 = new Point((p3.x+p2.x)/2, (p3.y+p2.y)/2, (p3.z+p2.z)/2);
    ...
    boolean is_point_inside_circumcircle(Point $p) {
      if (ccp.get_distance_to($p)<=ccr) { 
        return true;
      }
      return false;
    }
    boolean is_line_intersecting(Line $l) {
      if (!pl.is_line_intersecting($l)) { 
        return false;
      }
      Point p; 
      Vector v1, v2, v3; 
      ...
      if (p==global.error_point) { 
        return false;
      }
      v1 = new Vector(p1.x-p.x, p1.y-p.y, p1.z-p.z); 
      v1.unitize();
      v2 = new Vector(p2.x-p.x, p2.y-p.y, p2.z-p.z); 
      v2.unitize();
      v3 = new Vector(p3.x-p.x, p3.y-p.y, p3.z-p.z); 
      v3.unitize();
      a1 = acos(min(max(dot_product(v1, v2), -1), 1));
      a2 = acos(min(max(dot_product(v2, v3), -1), 1));
      a3 = acos(min(max(dot_product(v3, v1), -1), 1));
      if (abs((a1+a2+a3)-TWO_PI)<=global.angle_tolerance) { 
        return true;
      }
      return false;
    }
    boolean is_segment_intersecting(Segment $s, boolean $inclusive) {
      ...
      float a1, a2, a3;
      p = pl.get_line_intersect_point(l);
      if ($s.get_distance_to(p.x, p.y, p.z)>global.distance_tolerance) { 
        return false;
      }
      v1 = new Vector(p1.x-p.x, p1.y-p.y, p1.z-p.z); 
      v1.unitize();
    ...
    boolean cull() {
      ...
      p1x = screenX(p1.x, p1.y, p1.z); 
      p1y = screenY(p1.x, p1.y, p1.z);
      p2x = screenX(p2.x, p2.y, p2.z); 
      p2y = screenY(p2.x, p2.y, p2.z);
      p3x = screenX(p3.x, p3.y, p3.z); 
      p3y = screenY(p3.x, p3.y, p3.z);
      if (((p2y-p1y)/(p2x-p1x)-(p3y-p1y)/(p3x-p1x)<0)^(p1x<=p2x==p1x>p3x)) { 
        return false;
      }
      return true;
    }
  ...
  class Global {
    ...
    void init() {
      ...
      ps[0] = new Point(-w/2, -h/2, 0); 
      ps[1] = new Point(w/2, -h/2, 0); 
      ps[2] = new Point(w/2, h/2, 0); 
      ps[3] = new Point(-w/2, h/2, 0);
      stage = new Polygon(ps);
      ps = new Point[3]; 
      ps[0] = new Point(0, -h/2-sqrt(sq(w)-sq(w/2)), 0); 
      ps[1] = new Point(-w/2-h/sqrt(3), h/2, 0); 
      ps[2] = new Point(w/2+h/sqrt(3), h/2, 0);
      circumscribed_face = new Face(ps);
      zero_point = new Point(0, 0, 0);
      error_point = new Point(-9999, -9999, -9999);
      sines = new float[angles]; 
      cosines = new float[angles];
  ...
  class Line {
    ...
    Segment get_segment_to_line(Line $l) {
      ...
      p13x = s.p1.x-$l.s.p1.x; 
      p13y = s.p1.y-$l.s.p1.y; 
      p13z = s.p1.z-$l.s.p1.z;
      p43x = $l.s.p2.x-$l.s.p1.x; 
      p43y = $l.s.p2.y-$l.s.p1.y; 
      p43z = $l.s.p2.z-$l.s.p1.z;
      p21x = s.p2.x-s.p1.x; 
      p21y = s.p2.y-s.p1.y; 
      p21z = s.p2.z-s.p1.z;
  ...
  class Plane {
    ...
    Plane(Point $p1, Point $p2, Point $p3) {
      ...
      p2 = $p2; 
      p3 = $p3;
      v12 = new Vector(p2.x-p1.x, p2.y-p1.y, p2.z-p1.z); 
      v23 = new Vector(p3.x-p2.x, p3.y-p2.y, p3.z-p2.z); 
      v31 = new Vector(p1.x-p3.x, p1.y-p3.y, p1.z-p3.z);
      nv = get_normal();
      d = get_d();
    }
    Vector get_normal() {
    ...
    boolean is_line_intersecting(Line $l) {
      ...
      if (pbmud==0) { 
        return false;
      } //plane and line do not intersect
      return true;
    }
    boolean is_segment_intersecting(Segment $s, boolean $inclusive) {
      float pbmud, pbmu;
    ...
    Point get_3plane_intersection_point(Plane $pl1, Plane $pl2) {
      ...
      Vector n12, n23, n31; 
      float px, py, pz, pbden;
      nv = get_normal();
      d = get_d();
      n12 = cross_product($pl1.nv, $pl2.nv); 
      n23 = cross_product($pl2.nv, nv); 
      n31 = cross_product(nv, $pl1.nv);
  ...
  class Polygon {
    ...
    void add_point(Point $p) {
      for (int i=0; i<npoints; i++) {
        if ($p.get_distance_to(points[i].x, points[i].y, points[i].z)<=global.distance_tolerance) { 
          return;
        }
      }
      points[npoints++] = $p;
    ...
    Polygon get_bounding_box() {
      ...
      Point[] ps = new Point[4]; 
      ps[0] = new Point(xmin, ymin, 0); 
      ps[1] = new Point(xmax, ymin, 0); 
      ps[2] = new Point(xmax, ymax, 0); 
      ps[3] = new Point(xmin, ymax, 0);
    ...
    boolean is_complex() {
      //xy coordinates only
      if (npoints!=nsegments) { 
        return true;
      }
      for (int i=0; i<nsegments; i++) {
        for (int j=i+1; j<nsegments; j++) {
          if (j==i+1||j==(i+nsegments-1)%nsegments) {
            if (segments[i].is_segment_intersecting(segments[j], false)) { 
              return true;
            }
          } else {
            if (segments[i].is_segment_intersecting(segments[j], true)) { 
              return true;
            }
          }
        }
    ...
    boolean is_coord_inside(float $x, float $y) {
      ...
      if (abs(a)+global.angle_tolerance<PI) { 
        return false;
      }
      return true;
    }
    boolean is_polygon_inside(Polygon $po) {
      //xy coordinates only
      for (int i=0; i<npoints; i++) {
        if (!is_coord_inside(points[i].x, points[i].y)) { 
          return false;
        }
      }
      return true;
    ...
    boolean is_polygon_intersecting(Polygon $po) {
      ...
      if (is_polygon_inside(this)) { 
        return true;
      }
      if (is_polygon_inside($po)) { 
        return true;
      }
      return false;
    }
    ...
    Polygon get_convex_hull() {
      ...
      int ip = 0; 
      int ntemp = 0; 
      Point[] temp = new Point[npoints]; 
      float[] as = new float[npoints]; 
      float[] ds = new float[npoints];
      //find the topmost point
      for (int i=1; i<npoints; i++) {
        if (points[i].y<points[ip].y) { 
          ip = i;
        }
      }
            ...
            for (int j=0; j<ntemp; j++) {
              if (a==as[j]) {
                if (d>ds[j]) {
                  ...
                  Point[] temp1 = (Point[]) subset(temp, 0, j); 
                  Point[] temp2 = (Point[]) subset(temp, j+1, temp.length-(j+1)); 
                  temp = (Point[]) concat(temp1, temp2);
                  float[] as1 = subset(as, 0, j); 
                  float[] as2 = subset(as, j+1, temp.length-(j+1)); 
                  as = concat(as1, as2);
                  float[] ds1 = subset(ds, 0, j); 
                  float[] ds2 = subset(ds, j+1, temp.length-(j+1)); 
                  ds = concat(ds1, ds2);
                  ntemp--; 
                  j--;
                } else {
    ...
    Polygon get_outline(float $cohesion_distance) {
      ...
      int ntemp = 0; 
      Point[] temp = new Point[npoints]; 
      float cosa = cos(-PI+0.001); 
      float sina = sin(-PI+0.001);
      //find the topmost point
      for (int i=0; i<npoints; i++) {
        if (points[i].y<points[ip].y) { 
          ip = i;
      ...
      while ( (active!=ip||n==0)&&n<100) {
        ...
        for (int j=0; j<npoints; j++) {
          if (j!=active&&points[active].get_distance_to(points[j])<=$cohesion_distance) {
            v.vx = points[j].x-points[active].x; 
            v.vy = points[j].y-points[active].y;
        ...
        ov.vx = points[next].x-points[active].x; 
        ov.vy = points[next].y-points[active].y;
    ...
    Polygon[] split(Line $l) {
      ...
      int ip = 0; 
      Point[] ints = new Point[npoints]; 
      float[] intsi = new float[npoints+2]; 
      int nints = 0; 
      Point[] temp1 = new Point[npoints+2]; 
      float[] temp1i = new float[npoints+2]; 
      int ntemp1 = 0; 
      Point[] temp2 = new Point[npoints+2]; 
      int ntemp2 = 0;
      for (int i=0; i<npoints; i++) {
        if ($l.is_coord_on(points[i].x, points[i].y, 0)) {
          ints[nints] = points[i]; 
      ...
      for (int i=ip; i<ip+npoints; i++) {
        ...
        if (!found) { 
          temp2[ntemp2++] = points[i%npoints];
        }
      }
      temp2[ntemp2] = new Point(0, 0, 0); 
      temp2[ntemp2++].match(ints[0]);
      temp2[ntemp2] = new Point(0, 0, 0); 
      temp2[ntemp2++].match(ints[1]);
  ...
  class Segment {
    ...
    boolean is_identical(Segment $s) {
      if ((p1==$s.p1&&p2==$s.p2)||(p2==$s.p1&&p1==$s.p2)) { 
        return true;
      }
      return false;
    }
    boolean is_coincident(Segment $s) {
      ...
      if (is_coord_on($s.p1.x, $s.p1.y, $s.p1.z)&&abs(slope1-slope2)<=global.slope_tolerance) { 
        return true;
      }
      return false;
    }
  ...
  class Shadow {
    ...
    void project() {
      Point sun = new Point(100, 0, 0); 
      sun.rotate(cos(sun_ry), sin(sun_ry), "y", 0, 0, 0); 
      sun.rotate(cos(sun_rz), sin(sun_rz), "z", 0, 0, 0);
          ...
          if (segments.length>0) {
            ...
            for (int i=0; i<segments.length; i++) {
              ...
              if (temp!=global.error_point) {
                ...
                if (temp!=global.error_point) { 
                  points = (Point[]) append(points, temp);
                }
              }
            }
  ...
  class Terrain {
    ...
    Terrain(float $x, float $y, float $z, float $w, float $h, float $d, float $density) {
      ...
      for (int i=0; i<xpoints-1; i++) {
        for (int j=0; j<ypoints-1; j++) {
          Point[] ps1 = new Point[3]; 
          Point[] ps2 = new Point[3];
          if ((i%2==0&&j%2==1)||(i%2==1&&j%2==0)) {
            ps1[0] = points[i][j]; 
            ps1[1] = points[i+1][j]; 
            ps1[2] = points[i][j+1];
            ps2[0] = points[i+1][j+1]; 
            ps2[1] = points[i][j+1]; 
            ps2[2] = points[i+1][j];
          } else {
            ps1[0] = points[i][j+1]; 
            ps1[1] = points[i][j]; 
            ps1[2] = points[i+1][j+1];
            ps2[0] = points[i+1][j]; 
            ps2[1] = points[i+1][j+1]; 
            ps2[2] = points[i][j];
          }
    ...
    void smooth(float $weight, int $iterations) {
      ...
      for (int i=0; i<xpoints; i++) {
        for (int j=0; j<ypoints; j++) {
          ...
          if (i>0) { 
            z += points[i-1][j].z*$weight/neighbors;
          }
          if (i<xpoints-1) { 
            z += points[i+1][j].z*$weight/neighbors;
          }
          if (j>0) { 
            z += points[i][j-1].z*$weight/neighbors;
          }
          if (j<ypoints-1) { 
            z += points[i][j+1].z*$weight/neighbors;
          }
    ...
    Face get_line_intersect_face(Line $l) {
      ...
      for (int i=0; i<xpoints-1; i++) {
        for (int j=0; j<ypoints-1; j++) {
          if (faces[i][j][0].is_line_intersecting($l)) { 
            return faces[i][j][0];
          }
          if (faces[i][j][1].is_line_intersecting($l)) { 
            return faces[i][j][1];
          }
        }
      }
      Point[] ps = {
        global.error_point, global.error_point, global.error_point
      };
      return new Face(ps);
    }
    Face[] get_line_intersect_faces(Line $l) {
      ...
      for (int i=0; i<xpoints-1; i++) {
        for (int j=0; j<ypoints-1; j++) {
          if (faces[i][j][0].is_line_intersecting($l)) { 
            fs = (Face[]) append(fs, faces[i][j][0]);
          }
          if (faces[i][j][1].is_line_intersecting($l)) { 
            fs = (Face[]) append(fs, faces[i][j][1]);
          }
        }
      }
      return fs;
    ...
    Segment[] get_edge_intersect_segments(Segment $s) {
      ...
      for (int i=0; i<xpoints-1; i++) {
        for (int j=0; j<ypoints-1; j++) {
          //test cross segment
          if ((i%2==0&&j%2==1)||(i%2==1&&j%2==0)) {
            temp.p1 = points[i][j+1]; 
            temp.p2 = points[i+1][j];
          } else {
            temp.p1 = points[i][j]; 
            temp.p2 = points[i+1][j+1];
          }
          if (temp.is_segment_intersecting($s, true)) { 
            segments[nsegments++] = new Segment(temp.p1, temp.p2);
          }
          //test right segment if on right grid square
          if (i==xpoints-2) {
            temp.p1 = points[i+1][j]; 
            temp.p2 = points[i+1][j+1];
            if (temp.is_segment_intersecting($s, true)) { 
              segments[nsegments++] = new Segment(temp.p1, temp.p2);
            }
